...
for (i in 1:nlayers(mpwp)) {
  r <- mpwp[[i]]
  name <- paste0('data/hindcast_layers/processed_WorldClim/mPWP/', names(mpwp)[i], '.bil')
...
for (i in 1:nlayers(mis)) {
  r <- mis[[i]]
  name <- paste0('data/hindcast_layers/processed_WorldClim/MIS19/', names(mis)[i], '.bil')
  writeRaster(r, filename = name, overwrite = T)
}
...
for (i in 1:nlayers(lig)) {
  r <- lig[[i]]
  name <- paste0('data/hindcast_layers/processed_WorldClim/lig/', names(lig)[i], '.bil')
  writeRaster(r, filename = name, overwrite = T)
}
...
for (i in 1:nlayers(lgm)) {
  r <- lgm[[i]]
  name <- paste0('data/hindcast_layers/processed_WorldClim/lgm/', names(lgm)[i], '.bil')
  writeRaster(r, filename = name, overwrite = T)
}
...
for (i in 1:nlayers(mh)) {
  r <- mh[[i]]
  name <- paste0('data/hindcast_layers/processed_WorldClim/mh/', names(mh)[i], '.bil')
  writeRaster(r, filename = name, overwrite = T)
}
