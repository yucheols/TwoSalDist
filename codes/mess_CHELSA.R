##### Run Multivariate Environmental Similarity Surface (MESS) // with CHELSA data ------------------------------------------------------------
...
# run
chelsa.mess <- get_mess(m_stack = envs, g_stack_list = list(mh, lgm, lig, mis, mpwp), 
                        names_mess = c('MH','LGM','LIG','MIS','mPWP'))

# export CHELSA MESS
for (i in 1:nlayers(chelsa.mess)) {
  r <- chelsa.mess[[i]]
  writeRaster(r, filename = paste0('output_other/CHELSA/clim_only/MESS/', names(chelsa.mess)[i], '_ntb_mess.tif'), overwrite = T)
}
