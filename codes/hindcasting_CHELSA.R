...
for (i in 1:nlayers(o.mess)) {
  r <- o.mess[[i]]
  name <- paste0('output_other/CHELSA/clim_only/MESS/O.koreanus/O.koreanus_', names(o.mess)[i], '_MESS.tif')
  writeRaster(r, filename = name, overwrite = T)
}
...
for (i in 1:nlayers(k.mess)) {
  r <- k.mess[[i]]
  name <- paste0('output_other/CHELSA/clim_only/MESS/K.koreana/K.koreana_', names(k.mess)[i], '_MESS.tif')
  writeRaster(r, filename = name, overwrite = T)
}
